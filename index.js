...
module.exports = function (grunt, cb) {
	...
	hooker.hook(grunt.log, 'header', function () {
		...
		if (name == 'force-internal:post:cucumberjs:all' || name=='force-internal:post:shell:parallel') {
			log("writing the text file")
			require('fs').writeFileSync('./report/time.txt', prettyMs(diff));
		}
		else
			log("Task Name: " + name + "Task Time :-" + prettyMs(diff))
	...
	process.once('timegruntexit', function (exitCode) {
		...
		require('fs').writeFileSync('./report/gruntTaskTimeStamp.txt', formatTable(tableData));
